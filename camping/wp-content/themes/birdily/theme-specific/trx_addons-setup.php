...
------------------------------------------------------------------------------- */
if ( ! function_exists( 'birdily_trx_addons_theme_specific_setup1' ) ) {
	add_filter( 'trx_addons_filter_components_editor', 'birdily_trx_addons_theme_specific_components' );
	function birdily_trx_addons_theme_specific_components( $enable = true ) {
		return BIRDILY_THEME_FREE
					? false     // Free version
					: true;     // Pro version or Developer mode
	}
}

if ( ! function_exists( 'birdily_trx_addons_theme_specific_setup1' ) ) {
	add_action( 'after_setup_theme', 'birdily_trx_addons_theme_specific_setup1', 1 );
	function birdily_trx_addons_theme_specific_setup1() {
		if ( birdily_exists_trx_addons() ) {
			add_filter( 'trx_addons_cv_enable', 'birdily_trx_addons_cv_enable' );
			add_filter( 'trx_addons_demo_enable', 'birdily_trx_addons_demo_enable' );
			add_filter( 'trx_addons_filter_edd_themes_market', 'birdily_trx_addons_edd_themes_market_enable' );
			add_filter( 'trx_addons_api_list', 'birdily_trx_addons_api_list' );
			add_filter( 'trx_addons_cpt_list', 'birdily_trx_addons_cpt_list' );
			add_filter( 'trx_addons_sc_list', 'birdily_trx_addons_sc_list' );
			add_filter( 'trx_addons_widgets_list', 'birdily_trx_addons_widgets_list' );
		}
	}
}

// CV
if ( ! function_exists( 'birdily_trx_addons_cv_enable' ) ) {
	
	function birdily_trx_addons_cv_enable( $enable = false ) {
...
// Demo mode
if ( ! function_exists( 'birdily_trx_addons_demo_enable' ) ) {
	
	function birdily_trx_addons_demo_enable( $enable = false ) {
...
// EDD Themes market
if ( ! function_exists( 'birdily_trx_addons_edd_themes_market_enable' ) ) {
	
	function birdily_trx_addons_edd_themes_market_enable( $enable = false ) {
...
// API
if ( ! function_exists( 'birdily_trx_addons_api_list' ) ) {
	
	function birdily_trx_addons_api_list( $list = array() ) {
		// To do: Enable/Disable Third-party plugins API via add/remove it in the list
		$list['wp-travel'] = array(
...
// CPT
if ( ! function_exists( 'birdily_trx_addons_cpt_list' ) ) {
	
	function birdily_trx_addons_cpt_list( $list = array() ) {
		// To do: Enable/Disable CPT via add/remove it in the list

		// If it's a free version - leave only basic set
		if ( BIRDILY_THEME_FREE ) {
			$free_cpt = array( 'layouts', 'portfolio', 'post', 'services', 'team', 'testimonials', 'itineraries' );
			foreach ( $list as $k => $v ) {
...
// Shortcodes
if ( ! function_exists( 'birdily_trx_addons_sc_list' ) ) {
	
	function birdily_trx_addons_sc_list( $list = array() ) {
		// To do: Add/Remove shortcodes into list
		// If you add new shortcode - in the theme's folder must exists /trx_addons/shortcodes/new_sc_name/new_sc_name.php
		$list['button']['layouts_sc']['player'] = esc_html__('Player', 'birdily');
		$list['button']['layouts_sc']['vertical'] = esc_html__('Vertical', 'birdily');
		// If it's a free version - leave only basic set
		if ( BIRDILY_THEME_FREE ) {
...
// Widgets
if ( ! function_exists( 'birdily_trx_addons_widgets_list' ) ) {
	
	function birdily_trx_addons_widgets_list( $list = array() ) {
...
if ( ! function_exists( 'birdily_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'birdily_trx_addons_menu_cache' );
	function birdily_trx_addons_menu_cache( $list = array() ) {
...
if ( ! function_exists( 'birdily_trx_addons_localize_script' ) ) {
	...
	function birdily_trx_addons_localize_script( $arr ) {
		$arr['alter_link_color'] = birdily_get_scheme_color( 'alter_link' );
		return $arr;
	}
}
...
if ( ! function_exists( 'birdily_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'birdily_trx_addons_sc_type', 10, 2 );
	function birdily_trx_addons_sc_type( $list, $sc ) {
		// To do: check shortcode slug and if correct - add new 'key' => 'title' to the list
		if ( 'trx_sc_blogger' == $sc ) {
			$list = birdily_array_merge( $list, birdily_get_list_blog_styles( false, 'sc' ) );
		}
		if ($sc == 'trx_sc_title') {
...
if ( ! function_exists( 'birdily_trx_addons_sc_prepare_atts' ) ) {
	add_filter( 'trx_addons_filter_sc_prepare_atts', 'birdily_trx_addons_sc_prepare_atts', 10, 2 );
	function birdily_trx_addons_sc_prepare_atts( $atts, $sc ) {
		if ( 'trx_sc_blogger' == $sc ) {
			$list = birdily_get_list_blog_styles( false, 'sc' );
...
if ( ! function_exists( 'birdily_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'birdily_trx_addons_sc_atts', 10, 2 );
	function birdily_trx_addons_sc_atts( $atts, $sc ) {
...
if ( ! function_exists( 'birdily_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'birdily_trx_addons_sc_map', 10, 2 );
	function birdily_trx_addons_sc_map( $params, $sc ) {
		...
		if ( in_array(
			...
			$params['params'][] = array(
				'param_name'  => 'scheme',
				'heading'     => esc_html__( 'Color scheme', 'birdily' ),
				'description' => wp_kses_data( __( 'Select color scheme to decorate this block', 'birdily' ) ),
				'group'       => esc_html__( 'Colors', 'birdily' ),
				'admin_label' => true,
				'value'       => array_flip( birdily_get_list_schemes( true ) ),
				'type'        => 'dropdown',
			);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading'          => esc_html__( 'Color style', 'birdily' ),
			'description'      => wp_kses_data( __( 'Select color style to decorate this block', 'birdily' ) ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label'      => true,
			'value'            => array_flip( birdily_get_list_sc_color_styles() ),
			'type'             => 'dropdown',
		);
		...
		if ( 'trx_sc_blogger' == $sc ) {
			...
			$param      = array(
				'param_name'       => 'hover',
				'heading'          => esc_html__( 'Image hover', 'birdily' ),
				'description'      => wp_kses_data( __( 'Select hover effect for the featured image', 'birdily' ) ),
				'edit_field_class' => 'vc_col-sm-4',
				'value'            => array_flip( birdily_get_list_hovers( true ) ),
				'dependency'       => array(
					'element' => 'type',
...
if ( ! function_exists( 'birdily_trx_addons_sow_map' ) ) {
	add_filter( 'trx_addons_sow_map', 'birdily_trx_addons_sow_map', 10, 2 );
	function birdily_trx_addons_sow_map( $params, $sc ) {

		// Param 'color_style'
		$param = array(
			'color_style' => array(
				'label'       => esc_html__( 'Color style', 'birdily' ),
				'description' => wp_kses_data( __( 'Select color style to decorate this block', 'birdily' ) ),
				'options'     => birdily_get_list_sc_color_styles(),
				'default'     => 'default',
				'type'        => 'select',
			),
		);
		if ( in_array( $sc, array( 'trx_sc_button' ) ) ) {
			birdily_array_insert_after( $params, 'size', $param );
		} elseif ( in_array(
			$sc, array(
				'trx_sc_action',
				'trx_sc_blogger',
			...
			birdily_array_insert_after( $params, 'title_align', $param );
		}
		return $params;
	}
}
...
if ( ! function_exists( 'birdily_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'birdily_trx_addons_sc_output', 10, 4 );
	function birdily_trx_addons_sc_output( $output, $sc, $atts, $content ) {

		if ( in_array( $sc, array( 'trx_sc_action' ) ) ) {
			if ( ! empty( $atts['scheme'] ) && ! birdily_is_inherit( $atts['scheme'] ) ) {
				$output = str_replace( 'class="sc_action ', 'class="sc_action scheme_' . esc_attr( $atts['scheme'] ) . ' ', $output );
			}
			if ( ! empty( $atts['color_style'] ) && ! birdily_is_inherit( $atts['color_style'] ) && 'default' != $atts['color_style'] ) {
...
if ( ! function_exists( 'birdily_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'birdily_trx_addons_sc_item_title_tag' );
	function birdily_trx_addons_sc_item_title_tag( $tag = '' ) {
		return 'h1' == $tag ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( ! function_exists( 'birdily_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'birdily_trx_addons_sc_item_button_args', 10, 3 );
	function birdily_trx_addons_sc_item_button_args( $args, $sc, $sc_args ) {
...
if ( ! function_exists( 'birdily_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title', 'birdily_trx_addons_slider_title', 10, 2 );
	function birdily_trx_addons_slider_title( $title, $data ) {
		$title = '';
		if ( ! empty( $data['title'] ) ) {
...
if ( ! function_exists( 'birdily_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles', 'birdily_trx_addons_sc_googlemap_styles' );
	function birdily_trx_addons_sc_googlemap_styles( $list ) {
		$list['dark'] = esc_html__( 'Dark', 'birdily' );
		return $list;
	}
}
...
if ( ! function_exists( 'birdily_trx_addons_tiny_mce_style_formats' ) ) {
	add_filter( 'trx_addons_filter_tiny_mce_style_formats', 'birdily_trx_addons_tiny_mce_style_formats' );
	function birdily_trx_addons_tiny_mce_style_formats( $style_formats ) {
		...
		if ( false && is_array( $style_formats ) && count( $style_formats ) > 0 ) {
			foreach ( $style_formats as $k => $v ) {
				if ( esc_html__( 'List styles', 'birdily' ) == $v['title'] ) {
					$style_formats[ $k ]['items'][] = array(
						'title'    => esc_html__( 'Arrow', 'birdily' ),
						'selector' => 'ul',
...
if ( ! function_exists( 'birdily_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'birdily_filter_allow_override_header_image', 'birdily_trx_addons_allow_override_header_image' );
	function birdily_trx_addons_allow_override_header_image( $allow ) {
		return birdily_is_team_page() ? false : $allow;
	}
}

// Get thumb size for the team items
if ( ! function_exists( 'birdily_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size', 'birdily_trx_addons_thumb_size', 10, 2 );
	function birdily_trx_addons_thumb_size( $thumb_size = '', $type = '' ) {
...
// Add fields to the theme option for the team members
// All other CPT theme options may be modified in the same method
if ( ! function_exists( 'birdily_trx_addons_override_options_fields' ) ) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'birdily_trx_addons_override_options_fields', 10, 2 );
	function birdily_trx_addons_override_options_fields( $mb, $post_type ) {
		if ( defined( 'TRX_ADDONS_CPT_TEAM_PT' ) && TRX_ADDONS_CPT_TEAM_PT == $post_type ) {
			if ( ! isset( $mb['email'] ) ) {
				$mb['email'] = array(
					'title'   => esc_html__( 'E-mail', 'birdily' ),
					'desc'    => wp_kses_data( __( "Team member's email", 'birdily' ) ),
					'std'     => '',
					'details' => true,
					'type'    => 'text',
				);
