...
if ( ! function_exists( 'birdily_need_page_title' ) ) {
	function birdily_need_page_title() {
		return ! is_front_page() && apply_filters( 'birdily_filter_need_page_title', true );
	}
}
...
if ( ! function_exists( 'birdily_show_layout' ) ) {
	function birdily_show_layout( $str, $before = '', $after = '' ) {
		if ( trim( $str ) != '' ) {
...
if ( ! function_exists( 'birdily_get_logo_image' ) ) {
	function birdily_get_logo_image( $type = '' ) {
		$logo_image = '';
		if ( birdily_is_on( birdily_get_theme_option( 'logo_retina_enabled' ) ) && birdily_get_retina_multiplier( 2 ) > 1 ) {
			$logo_image = birdily_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) . '_retina' );
		}
		if ( empty( $logo_image ) ) {
			if ( empty( $type ) && function_exists( 'the_custom_logo' ) ) {
				...
				$logo_image = birdily_get_theme_option( 'logo' . ( ! empty( $type ) ? '_' . trim( $type ) : '' ) );
			}
		}
		return birdily_remove_protocol_from_url( $logo_image, false );
	}
}

// Return header video (if set)
if ( ! function_exists( 'birdily_get_header_video' ) ) {
	function birdily_get_header_video() {
		$video = '';
		if ( apply_filters( 'birdily_header_video_enable', ! wp_is_mobile() && is_front_page() ) ) {
			if ( birdily_check_theme_option( 'header_video' ) ) {
				$video = birdily_get_theme_option( 'header_video' );
...
if ( ! function_exists( 'birdily_show_post_meta' ) ) {
	function birdily_show_post_meta( $args = array() ) {
		if ( is_single() && birdily_is_off( birdily_get_theme_option( 'show_post_meta' ) ) ) {
		...
		<div class="post_meta">
			...
			foreach ( $components as $comp ) {
				...
				if ( 'categories' == $comp ) {
					// Post categories
					$cats = get_post_type() == 'post' ? get_the_category_list( ' ' ) : apply_filters( 'birdily_filter_get_post_categories', '' );
					if ( ! empty( $cats ) ) {
						?>
						<span class="post_meta_item post_categories"><?php birdily_show_layout( $cats ); ?></span>
						<?php
					}
				} elseif ( 'date' == $comp ) {
					// Post date
					$dt = apply_filters( 'birdily_filter_get_post_date', birdily_get_date() );
					if ( ! empty( $dt ) ) {
						?>
					...
					// Post counters
					$output = birdily_get_post_counters( $args['counters'] );
					if ( '' != $output ) {
						birdily_show_layout( $output );
					} elseif ( ! birdily_exists_trx_addons() ) {
						if ( ! is_singular() || have_comments() || comments_open() ) {
							...
							?>
								<a href="<?php comments_link(); ?>" class="post_meta_item post_counters_item post_counters_comments icon-comment-light"><span class="post_counters_number">
									...
									<?php
									echo esc_html( _n( 'Comment', 'Comments', $post_comments, 'birdily' ) );
									?>
									</span>
								</a>
							<?php
					...
					birdily_show_share_links(
						array(
							'type'    => 'drop',
							'caption' => esc_html__( 'Share', 'birdily' ),
					...
					// Edit page link
					edit_post_link( esc_html__( 'Edit', 'birdily' ), '', '', 0, 'post_meta_item post_edit icon-pencil' );
				} else {
					// Custom counter
					do_action( 'birdily_action_show_post_counter', $comp, get_the_ID() );
				}
			}
			?>
		</div><!-- .post_meta -->
...
if ( ! function_exists( 'birdily_show_post_featured' ) ) {
	function birdily_show_post_featured( $args = array() ) {
		$args = array_merge(
			array(
				'hover'         => birdily_get_theme_option( 'image_hover' ), // Hover effect
		...
		$thumb_size  = ! empty( $args['thumb_size'] )
						? $args['thumb_size']
						: birdily_get_thumb_size( is_attachment() || is_single() ? 'full' : 'big' );
		$post_format = str_replace( 'post-format-', '', get_post_format() );
		$no_image    = ! empty( $args['show_no_image'] ) ? birdily_get_no_image() : '';
		if ( $args['thumb_bg'] ) {
			if ( has_post_thumbnail() ) {
				$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), $thumb_size );
				$image = empty( $image[0] ) ? '' : $image[0];
			} elseif ( 'image' == $post_format ) {
				$image = birdily_get_post_image();
				if ( ! empty( $image ) ) {
					$image = birdily_add_thumb_size( $image, $thumb_size );
				}
			}
			if ( empty( $image ) ) {
				$image = $no_image;
			...
			if ( ! empty( $image ) ) {
				$args['class'] .= ( $args['class'] ? ' ' : '' ) . 'post_featured_bg' . ' ' . birdily_add_inline_css_class( 'background-image: url(' . esc_url( $image ) . ');' );
			}
		}
		...
		if ( $args['singular'] ) {
			...
			if ( is_attachment() ) {
				...
				if ( ! $args['thumb_bg'] ) {
					echo wp_get_attachment_image(
						...
						birdily_is_on( birdily_get_theme_option( 'seo_snippets' ) )
													? array( 'itemprop' => 'image' )
						: ''
					);
				}
				if ( birdily_get_theme_setting( 'attachments_navigation' ) ) {
					?>
						<nav id="image-navigation" class="navigation image-navigation">
							<div class="nav-previous">Prev<br>post<?php previous_image_link( false, '' ); ?></div>
							<div class="nav-next">Next<br>post<?php next_image_link( false, '' ); ?></div>
				...
				if ( has_post_thumbnail() && $args['seo'] ) {
					$birdily_attr = birdily_getimagesize( wp_get_attachment_url( get_post_thumbnail_id() ) );
					?>
						<meta itemprop="width" content="<?php echo esc_attr( $birdily_attr[0] ); ?>">
						<meta itemprop="height" content="<?php echo esc_attr( $birdily_attr[1] ); ?>">
						<?php
				}
				if ( ! $args['thumb_bg'] ) {
					if ( has_post_thumbnail() ) {
			...
			if ( $has_thumb	|| ! empty( $args['show_no_image'] )
				...
				if ( $args['thumb_bg'] ) {
					...
					$output = birdily_get_slider_layout( $slider_args );
					if ( '' != $output ) {
						birdily_show_layout( $output );
					}
					//------- End: Moved down --------
				...
				if ( ! $args['thumb_only'] && ( in_array( $post_format, array( 'video', 'audio', 'gallery' ) ) ) ) {
					$post_content = birdily_get_post_content();
					$post_content_parsed = $post_content;
					// Put video under the thumb
					if ( 'video' == $post_format ) {
						$video = birdily_get_post_video( $post_content, false );
						if ( empty( $video ) ) {
							$video = birdily_get_post_iframe( $post_content, false );
						}
						if ( empty( $video ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $videos ) && is_array( $videos ) ) {
									$video = birdily_array_get_first( $videos, false );
								}
							}
						}
						if ( ! empty( $video ) ) {
							if ( $has_thumb ) {
								$video = birdily_make_video_autoplay( $video );
								?>
						...
						$audio = birdily_get_post_audio( $post_content, false );
						if ( empty( $audio ) ) {
							$audio = birdily_get_post_iframe( $post_content, false );
						}
						...
						if ( empty( $audio ) ) {
							...
							if ( false === strpos( $post_content_parsed, 'wp-playlist-script' ) ) {
								...
								if ( ! empty( $audios ) && is_array( $audios ) ) {
									$audio = birdily_array_get_first( $audios, false );
								}
							}
						}
						if ( ! empty( $audio ) ) {
							...
							">
								...
								$media = urldecode( birdily_get_tag_attrib( $post_content, '[trx_widget_audio]', 'media' ) );
								$media_author = '';
								$media_title  = '';
								if ( ! empty( $media ) ) {
									...
									// Parse tag params
									$media_author = strip_tags( birdily_get_tag( $post_content_parsed, '<h6 class="audio_author">', '</h6>' ) );
									$media_title  = strip_tags( birdily_get_tag( $post_content_parsed, '<h5 class="audio_caption">', '</h5>' ) );

								}
								if ( ! empty( $media_author ) ) {
									?>
									<div class="post_audio_author"><?php birdily_show_layout( $media_author ); ?></div>
									<?php
								}
								if ( ! empty( $media_title ) ) {
									?>
									<h5 class="post_audio_title"><?php birdily_show_layout( $media_title ); ?></h5>
									<?php
								}
								// Display audio
								birdily_show_layout( $audio );
								?>
							</div>
							<?php
						}
				...
				birdily_show_layout( $post_info );
				// Put post categories				
				echo ! empty($args['post_categories']) ? trim($args['post_categories']) : '';
...
if ( ! function_exists( 'birdily_get_no_image' ) ) {
	function birdily_get_no_image( $no_image = '' ) {
		static $img = '';
		if ( empty( $img ) ) {
			$img = birdily_get_theme_option( 'no_image' );
			if ( empty( $img ) ) {
				$img = birdily_get_file_url( 'images/no-image.jpg' );
			}
		}
		if ( ! empty( $img ) ) {
			$no_image = $img;
...
if ( ! function_exists( 'birdily_add_bg_in_post_nav' ) ) {
	function birdily_add_bg_in_post_nav() {
		if ( ! is_single() ) {
			return;
		}
		...
		if ( $previous ) {
			if ( has_post_thumbnail( $previous->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $previous->ID ), birdily_get_thumb_size( 'med' ) );
				$img = empty( $img[0] ) ? '' : $img[0];;
			} elseif ( birdily_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
		...
		if ( $next ) {
			if ( has_post_thumbnail( $next->ID ) ) {
				$img = wp_get_attachment_image_src( get_post_thumbnail_id( $next->ID ), birdily_get_thumb_size( 'med' ) );
				$img = empty( $img[0] ) ? '' : $img[0];;
			} elseif ( birdily_get_theme_setting( 'allow_no_image' ) ) {
				$img = $noimg;
			}
		...
		wp_add_inline_style( 'birdily-main', $css );
	}
}

// Show related posts
if ( ! function_exists( 'birdily_show_related_posts' ) ) {
	function birdily_show_related_posts( $args = array(), $style = 1, $title = '' ) {
		$args = array_merge(
			array(
		...
		$taxonomy = 'post' == $args['post_type'] ? 'category' : birdily_get_post_type_taxonomy();
		...
		if ( $query->found_posts > 0 ) {
			...
			$args['slider'] = (int) birdily_get_theme_option( 'related_slider' ) && min( $args['posts_per_page'], $query->found_posts) > $columns;
			?>
			<section class="related_wrap">
				<h4 class="section_title related_wrap_title"><?php
				if ( ! empty( $title ) ) {
					echo esc_html( $title );
				} else {
					esc_html_e( 'You May Also Like', 'birdily' );
				}
				?></h4><?php
				if ( $args['slider'] ) {
					$slider_args                      = $args;
					...
					$slider_args['slides_space']      = birdily_get_theme_option( 'related_slider_space' );
					$slider_args['slider_controls']   = birdily_get_theme_option( 'related_slider_controls' );
					$slider_args['slider_pagination'] = birdily_get_theme_option( 'related_slider_pagination' );
					$slider_args                      = apply_filters( 'birdily_related_posts_slider_args', $slider_args, $query );
					?><div class="related_wrap_slider"><?php
					birdily_get_slider_wrap_start('related_posts_wrap', $slider_args);
				} else {
					...
					while ( $query->have_posts() ) {
						...
						get_template_part( apply_filters( 'birdily_filter_get_template_part', 'templates/related-posts', $style ), $style );
						?></div><?php
					}
				?></div><?php		
				if ( $args['slider'] ) {
					birdily_get_slider_wrap_end('related_posts_wrap', $slider_args);
...
if ( ! function_exists( 'birdily_show_related_posts_callback' ) ) {
	add_action( 'birdily_action_related_posts', 'birdily_show_related_posts_callback' );
	function birdily_show_related_posts_callback() {
		if ( is_single() && ! apply_filters( 'birdily_filter_show_related_posts', false ) ) {
			$birdily_related_posts    = (int) birdily_get_theme_option( 'related_posts' );
			if ( (int) birdily_get_theme_option( 'show_related_posts' ) && $birdily_related_posts > 0 ) {
				birdily_show_related_posts(
					array(
						'orderby'        => 'rand',
						'posts_per_page' => max( 1, min( 9, $birdily_related_posts ) ),
						'columns'        => max( 1, min( 4, birdily_get_theme_option( 'related_columns' ) ) ),
					),
					birdily_get_theme_option( 'related_style' )
				);
			}
		}
	}
...
if ( ! function_exists( 'birdily_show_portfolio_posts' ) ) {
	function birdily_show_portfolio_posts( $args = array() ) {
		$args = array_merge(
			array(
				'cat'        => 0,
				'parent_cat' => 0,
		...
		$blog_style = explode( '_', empty( $args['blog_style'] ) ? birdily_get_theme_option( 'blog_style' ) : $args['blog_style'] );
		$style      = $blog_style[0];
		...
		if ( ! $args['echo'] ) {
			...
			$q_args = birdily_query_add_posts_and_cats( $q_args, '', $args['post_type'], $args['cat'], $args['taxonomy'] );
			if ( $args['page'] > 1 ) {
				$q_args['paged']               = $args['page'];
				$q_args['ignore_sticky_posts'] = true;
			}
			$ppp = birdily_get_theme_option( 'posts_per_page' );
			if ( 0 != (int) $ppp ) {
		...
		while ( have_posts() ) {
			...
			$birdily_part = $args['sticky'] && is_sticky() ? 'sticky' : ( 'gallery' == $style ? 'portfolio-gallery' : $style );
...
// AJAX handler for the birdily_ajax_get_posts action
if ( ! function_exists( 'birdily_ajax_get_posts_callback' ) ) {
	add_action( 'wp_ajax_birdily_ajax_get_posts', 'birdily_ajax_get_posts_callback' );
	add_action( 'wp_ajax_nopriv_birdily_ajax_get_posts', 'birdily_ajax_get_posts_callback' );
	function birdily_ajax_get_posts_callback() {
		...
		if ( empty( $response['data'] ) ) {
			$response['error'] = esc_html__( 'Sorry, but nothing matched your search criteria.', 'birdily' );
		}
...
// Show pagination
if ( ! function_exists( 'birdily_show_pagination' ) ) {
	function birdily_show_pagination() {
		global $wp_query;
		// Pagination
		$pagination = birdily_get_theme_option( 'blog_pagination' );
		if ( 'pages' == $pagination ) {
			the_posts_pagination(
				array(
					'mid_size'           => 2,
					'prev_text'          => esc_html__( '<', 'birdily' ),
					'next_text'          => esc_html__( '>', 'birdily' ),
					'before_page_number' => '<span class="meta-nav screen-reader-text">' . esc_html__( 'Page', 'birdily' ) . ' </span>',
				)
			);
			...
			if ( $page_number < $wp_query->max_num_pages ) {
				...
				">
					<a class="nav-load-more" href="#" 
						...
						><span><?php esc_html_e( 'Load more posts', 'birdily' ); ?></span></a>
				</div>
				<?php
			}
			...
			<div class="nav-links-old">
				<span class="nav-prev"><?php previous_posts_link( is_search() ? esc_html__( 'Previous posts', 'birdily' ) : esc_html__( 'Newest posts', 'birdily' ) ); ?></span>
				<span class="nav-next"><?php next_posts_link( is_search() ? esc_html__( 'Next posts', 'birdily' ) : esc_html__( 'Older posts', 'birdily' ), $wp_query->max_num_pages ); ?></span>
			</div>
			<?php
		}
	}
...
if ( ! function_exists( 'birdily_single_comments_field' ) ) {
	function birdily_single_comments_field( $args ) {
		...
		if ( 'text' == $args['field_type'] ) {
			...
			$html .= '<input id="' . esc_attr( $args['field_name'] ) . '" name="' . esc_attr( $args['field_name'] ) . '" type="checkbox" value="' . esc_attr( $args['field_value'] ) . '"' . ( $args['field_req'] ? ' aria-required="true"' : '' ) . ' />'
					. ' <label for="' . esc_attr( $args['field_name'] ) . '" class="' . esc_attr( $args['field_req'] ? 'required' : 'optional' ) . '">' . wp_kses( $args['field_title'], 'birdily_kses_content' ) . '</label>';
		} else {
		...
		if ( 'default' != $args['form_style'] ) {
			$html .= '<span class="sc_form_field_hover">'
						...
						. '<span class="sc_form_field_content" data-content="' . esc_attr( $args['field_title'] ) . '">' . wp_kses( $args['field_title'],'birdily_kses_content' ) . '</span>'
					. '</span>';
		}
		$html .= '</span></div>';
		return $html;
