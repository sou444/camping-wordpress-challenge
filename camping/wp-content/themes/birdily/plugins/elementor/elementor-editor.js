	...
	function() {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="birdily_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.birdily_post_editor' );
					if ( link.length > 0 ) {
						birdily_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.birdily_post_editor', function(e) {
					birdily_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('birdily_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function birdily_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
