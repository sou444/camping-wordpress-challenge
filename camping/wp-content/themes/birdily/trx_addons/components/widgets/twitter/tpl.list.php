...
?><div class="widget_content"><ul class="sc_twitter sc_twitter_list"><?php
	...
	if (is_array($widget_args['data']) && count($widget_args['data']) > 0) {
		foreach ($widget_args['data'] as $tweet) {
			...
			?><li<?php if ($cnt==$twitter_count-1) echo ' class="last"'; ?>><a href="<?php echo esc_url('https://twitter.com/'.trim($twitter_username)); ?>" class="username" target="_blank">@<?php echo esc_html($tweet['user']['screen_name']); ?></a><br> <?php
...
if ($follow) {
	?><a href="<?php echo esc_url('https://twitter.com/'.trim($twitter_username)); ?>" class="widget_twitter_follow"><?php esc_html_e('Follow us', 'birdily'); ?></a><?php
}
