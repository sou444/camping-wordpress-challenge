...
?><div <?php post_class( 'sc_blogger_item post_format_'.esc_attr($post_format) . (empty($post_link) ? ' no_links' : '') ); ?>><?php
	...
	trx_addons_get_template_part('templates/tpl.featured.php',
									...
									apply_filters('trx_addons_filter_args_featured', array(
														...
														'thumb_size' => apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size((int)$args['columns'] > 2 ? 'medium' : 'blogger'), 'blogger-default')
														), 'blogger-default')
								);
		...
		if (!isset($args['hide_excerpt']) || (int)$args['hide_excerpt']==0) {
			?><div class="sc_blogger_item_excerpt">
				...
				if ( $show_more && !empty($post_link) && !empty($args['more_text']) ) {
					?><a class="more-link sc_button_hover_slide_left sc_button" href="<?php echo esc_url($post_link); ?>"><?php
