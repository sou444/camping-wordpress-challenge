<div class="front_page_section front_page_section_blog<?php
	$birdily_scheme = birdily_get_theme_option( 'front_page_blog_scheme' );
	if ( ! birdily_is_inherit( $birdily_scheme ) ) {
		echo ' scheme_' . esc_attr( $birdily_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( birdily_get_theme_option( 'front_page_blog_paddings' ) );
?>"
		<?php
		$birdily_css      = '';
		$birdily_bg_image = birdily_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$birdily_anchor_icon = birdily_get_theme_option( 'front_page_blog_anchor_icon' );
	$birdily_anchor_text = birdily_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( birdily_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$birdily_css      = '';
			$birdily_bg_mask  = birdily_get_theme_option( 'front_page_blog_bg_mask' );
			$birdily_bg_color_type = birdily_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $birdily_bg_color_type ) {
				$birdily_bg_color = birdily_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$birdily_caption = birdily_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $birdily_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $birdily_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $birdily_caption,'birdily_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$birdily_description = birdily_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $birdily_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $birdily_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $birdily_description ), 'birdily_kses_content' ); ?></div>
				<?php
			}
			...
			if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
				...
				if ( ! birdily_exists_trx_addons() ) {
					...
					birdily_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
				}
			}
			?>
			</div>
