...
class Essential_Grid_Update
{
	...
	protected function _check_updates()
	{
		...
		// Check for updates
		if (time() - $last_check > 172800 || $force == true || empty($this->data)) {
	...
	public function _retrieve_update_info()
	{
		...
		$request = $esg_loadbalancer->call_url($this->remote_url_info, $data, 'updates');
	...
	public function _retrieve_version_info($force = false)
	{
		...
		// Check for updates
		if ($last_check == false || time() - $last_check > 172800 || $force == true) {
			...
			$request = $esg_loadbalancer->call_url($this->remote_url, $data, 'updates');
			if (is_wp_error($request)) {
