...
class Essential_Grid_Navigation
{
	...
	static $grid_filter_count = array();
	...
	{
		return str_replace('%ESG-GRID-SERIAL%', $this->get_grid_filter_count($this->grid_id), $this->special_class);
	}
	 ...
	 * @return array
	 */
	public function get_grid_filter_count($grid_id)
	{
		if (!isset(self::$grid_filter_count[$grid_id])) {
			self::$grid_filter_count[$grid_id] = 0;
		}

		return self::$grid_filter_count[$grid_id];
	}

	/**
	 * @param mixed $grid_id
	 */
	public function update_grid_filter_count($grid_id)
	{
		if (!isset(self::$grid_filter_count[$grid_id])) {
			self::$grid_filter_count[$grid_id] = 0;
		} else {
			self::$grid_filter_count[$grid_id]++;
		}
	}
